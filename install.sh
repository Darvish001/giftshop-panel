...
REPO_URL="https://raw.githubusercontent.com/Darvish001/giftshop-panel/main"
DOCKER_IMAGE="darvish021/giftshop-panel:latest"
...
pull_and_run() {
    ...
    print_status "Starting GiftShop Panel..."
    docker compose up -d
...
install_command() {
    cat > /usr/local/bin/giftshop-panel << 'SCRIPT'
#!/bin/bash
...
case "$1" in
    ...
    uninstall)
        ...
        ;;
    *)
        echo "Usage: giftshop-panel {edit-env|update|start|stop|restart|logs|uninstall}"
        ;;
esac
SCRIPT
    chmod +x /usr/local/bin/giftshop-panel
}
